...
const FormUnit = ({ title, onChange }) => {
  return (
    <>
      ...
      <Input type="text" name="data" onChange={onChange} />
    </>
  );
};
...
function NewDataForm(props) {
  ...
  const [Inputs, setInputs] = useState([{ title: "Label" }]);
  ...
  const AddInputField = () => {
    setInputs((prev) => [...prev, { title: "New Label" }]);
  };
    ...
    <Form onSubmit={props.data ? editData : createData} ref={formRef}>
      <FormGroup>
        {Inputs.map((data, index) => {
          return (
            <FormUnit key={index} title={data.title} onChange={onChange} />
          );
        })}
        {props.create ? (
          <Button style={{float: "right", clear: "both", marginTop: "20px"}} onClick={() => AddInputField()}>+</Button>
        ) : (
          ""
        )}
      </FormGroup>
